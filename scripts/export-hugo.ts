...
import { subtypeOf } from "./vault-identity";
...
function resolveOutputPath(
    ...
    if (T === "doc" || SETTING_TYPES.has(T) || SOHL_SYSTEM_TYPES.has(T)) {
        ...
        // published by the Song-of-Heroic-Lands-FoundryVTT repository to
        // kb.heroiclands.org/{type}/{slug}/, which is generated by the code
...
const HUGO_FIELDS: Record<string, (fm: Record<string, any>) => any> = {
    ...
    // The note's identity, and the key every cross-page reference resolves on
    // (#1388). Distinct from `slug`, which is the URL segment: identity has to
    // survive a rename, and a URL exists to be improved. Emitted for every
    // note so the geo infoboxes can join on it — see the theme's
    // `partials/ref/page.html`.
...
/**
 * Emit `data/refnames.yaml` — shortcode → display name, for **every** vault
 * note, published or not.
 *
 * The geo infoboxes resolve a reference to a page and link it. When no page
 * exists they still have to render *something*, and until now that something
 * was the reference value run through `humanize` — which worked only because
 * the value was a slug: `aureldian-pantheon` reads back as "Aureldian
 * Pantheon". Now that references carry shortcodes (#1388), the same fallback
 * would print "Arldnpnthn".
 *
 * That fallback is not a rare path. Pantheons are authored as
 * `type: affiliation`, and the exporter routes only a couple of affiliation
 * notes, so ~300 pantheon rows across the setting have always rendered as
 * fallback text. This index keeps them reading correctly — better than before,
 * in fact, since it carries the authored name with its diacritics rather than
 * a de-slugified guess.
 *
 * Unpublished notes are exactly the ones that matter here, so this reads the
 * vault directly rather than the exported entries.
 *
 * @param dryRun - When true, report what would be written and write nothing.
 * @param verbose - Log the entry counts.
 */
function writeRefNames(dryRun: boolean, verbose: boolean): void {
    /** `subtype|shortcode` → display name. */
    const byId: Record<string, string> = {};
    /** `shortcode` → display name, only where the shortcode names one note. */
    const byShortcode: Record<string, string> = {};
    const ambiguous = new Set<string>();

    for (const filepath of findNotes(VAULT_ROOT)) {
        let fm: Record<string, any> | null;
        try {
            fm = readFrontMatter(fs.readFileSync(filepath, "utf8"));
        } catch {
            continue;
        }
        if (!fm) continue;
        ...
        const shortcode =
            typeof fm.shortcode === "string" ? fm.shortcode : "";
        if (!shortcode) continue;
        ...
        const subtype = subtypeOf(fm);
        if (subtype) byId[`${subtype}|${shortcode}`] = name;

        // A shortcode is unique within its type, not across types, so the
        // unqualified map keeps only the ones that name a single note.
        if (shortcode in byShortcode) ambiguous.add(shortcode);
        else byShortcode[shortcode] = name;
    }
    for (const key of ambiguous) delete byShortcode[key];

    if (verbose) {
        console.log(
            `  ${Object.keys(byId).length} qualified, ${Object.keys(byShortcode).length} unqualified` +
                `${ambiguous.size ? `, ${ambiguous.size} shortcode(s) shared across types` : ""}`,
        );
    }
    if (dryRun) return;

    const dataDir = path.join(HUGO_ROOT, "data");
    fs.mkdirSync(dataDir, { recursive: true });
    fs.writeFileSync(
        path.join(dataDir, "refnames.yaml"),
        "# Generated by scripts/export-hugo.ts — do not edit.\n" +
            yaml.dump({ byId, byShortcode }, { sortKeys: true, lineWidth: -1 }),
        "utf8",
    );
}
...
function main(): void {
    ...
    // Emit the reference-name index the theme falls back to when a reference
    // names a note that has no page of its own.
    console.log("\nWriting reference-name index...");
    writeRefNames(dryRun, verbose);
