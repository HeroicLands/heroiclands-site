 ...
 *
 * Shared by the passes that read or rewrite the vault — the legacy-URL capture
 * and the one-shot `slug` removal — which is why it lives apart from either.
 */
...
/**
 * A note's display name — the thing its URL is derived from.
 *
 * Three sources in order, which is the chain the exporter has always used to
 * title a page: the structured `name.full`, an unstructured `title`, and
 * failing both the filename with its underscores opened out.
 *
 * The fallbacks are not decoration. Six content notes carry no `name:` block at
 * all — `Setting/Lore/Divine_Action.md` and friends — and a rule that insisted
 * on `name.full` would leave them with no address the moment the authored
 * `slug` is removed. Their titles and filenames already derive exactly the URLs
 * they publish at today.
 *
 * @param fm - The note's front matter.
 * @param filepath - Its path, for the filename fallback.
 */
export function noteName(fm: Record<string, any>, filepath: string): string {
    const full = fm?.name?.full;
...
/**
 * A note's *subtype* — the segment that appears in its URL.
 *
 ...
 * `single.html` makes and the same one the exporter routes by, kept identical
 * on purpose.
 */
